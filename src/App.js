...
import { Posts } from "feed-dac-library/dist/cjs/skystandards";
...
function App() {
  ...
  const loadData = async (event) => {
    ...
    const postsLoader = feedDAC.loadPostsForUser(userID);
    const firstPage = await postsLoader.next();
    // console.log(`grabbing items in DAC ${firstPage.value}`);

    posts.push(...firstPage.value);
    console.log([...posts]);
